...
import org.reactome.server.graph.curator.domain.model.ReferenceMolecule;
import org.reactome.server.graph.curator.domain.model.ReferenceSequence;
import org.reactome.server.graph.curator.domain.result.ComponentOf;
import org.reactome.server.graph.curator.service.AdvancedLinkageService;
import org.reactome.server.graph.curator.service.PhysicalEntityService;
import org.reactome.server.graph.curator.service.SchemaService;
...
public class PhysicalEntityController {
    ...
    public String getReferenceMoleculesSummary() {
        List<String> rtn = schemaService.getByClass(ReferenceMolecule.class).stream().map(r -> r.getStId() + "\t" + r.getReferenceDatabase().getName() + ":" + r.getIdentifier()).collect(Collectors.toList());
        infoLogger.info("Request total list of ReferenceMolecules");
        return String.join("\n", rtn);
    }
    ...
    public String getReferenceSequencesSummary(@Parameter(description = "Page number", example = "1")
                                               ...
                                               @Parameter(description = "Number of elements per page", example = "20")
                                               @RequestParam Integer offset) {
        List<String> rtn = schemaService.getByClass(ReferenceSequence.class, page, offset).stream().map(r -> r.getStId() + "\t" + r.getReferenceDatabase().getName() + ":" + r.getIdentifier()).collect(Collectors.toList());
        infoLogger.info("Request total list of ReferenceSequences");
        return String.join("\n", rtn);
    }
}
